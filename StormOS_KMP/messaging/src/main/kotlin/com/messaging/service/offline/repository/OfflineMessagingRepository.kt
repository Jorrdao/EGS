 ...
 */
class OfflineMessagingRepository constructor(
    ...
    // Internal scope for fire-and-forget DB writes (e.g. persisting incoming BLE messages).
    // Cancelled when the service is destroyed via MessagingServiceLocator.cancel().
    ...
    private fun handleMeshMessage(fromAddress: String, envelope: BleEnvelope) {
        try {
            ...
            if (msg.destinationId == myUserId) {
                // This message is for us — decrypt and persist to Room DB
                val plaintext = String(
                    encryption.decrypt(msg.payload.toByteArray()),
                    Charsets.UTF_8
                )

                // Derive the chat_id the same way the UI does:
                // "chat_" + sorted(senderId, recipientId).join("_")
                val chatId = "chat_" + listOf(msg.senderId, msg.destinationId)
                    .sorted()
                    .joinToString("_")
                ...
                // insertOrIgnore prevents duplicates if the message
                // arrives more than once via different mesh hops
                repoScope.launch {
